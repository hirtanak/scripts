...
if [ $# -eq 0 ]; then
	...
	echo " - stopvm <start vm#> <end vm#>: コンピュートノードVM# xからVM# yまで停止します。" 1>&2
	...
	echo " - deletevm <vm#>: 特定のコンピュートノード#のみを削除します。" 1>&2
	exit 1
fi
# SSH鍵チェック。なければ作成
...
#		done
		...
		az vm list-ip-addresses -g $MyResourceGroup --query "[].virtualMachine[].{Name:name, PublicIp:network.publicIpAddresses[0].ipAddress}" -o tsv > tmpfile
		cat tmpfile | cut -f 2 > ipaddresslist
#		for count in $(seq 1 $MAXVM) ; do
...
#		sed -i -e '/^$/d' ./ipaddresslist
		...
		parallel $DEBUG -a ipaddresslist-tmp "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo yum install --quiet -y nfs-utils epel-release""
		parallel $DEBUG -a ipaddresslist-tmp "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo yum install --quiet -y htop""
		parallel $DEBUG -a ipaddresslist-tmp "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo mkdir -p /mnt/resource""
		parallel $DEBUG -a ipaddresslist-tmp "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo chown $USERNAME:$USERNAME /mnt/resource""
		parallel $DEBUG -a ipaddresslist-tmp "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo mount $DEBUG -t nfs ${mountip}:/mnt/resource /mnt/resource""
		# NFSサーバ・マウント設定
		...
		for count in $(seq 2 $MAXVM); do
			...
			if [ ! -z "$checkssh" ]; then
				echo "${VMPREFIX}-2 to ${MAXVM}: setting by ssh command"
				...
				ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${line} -t -t "echo '${mountip}:/mnt/resource    /mnt/resource    nfs    defaults    0    0' | sudo tee -a /etc/fstab"
			else
				echo "${VMPREFIX}-2 to ${MAXVM}: setting by az vm run-command"
...
EOL
		...
		parallel $DEBUG -a ipaddresslist "scp -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} ./${VMPREFIX} $USERNAME@{}:/home/$USERNAME/.ssh/${VMPREFIX}"
		parallel $DEBUG -a ipaddresslist "scp -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} ./${VMPREFIX} $USERNAME@{}:/home/$USERNAME/.ssh/id_rsa"
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "chmod 600 /home/$USERNAME/.ssh/id_rsa""
		parallel $DEBUG -a ipaddresslist "scp -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} ./${VMPREFIX}.pub $USERNAME@{}:/home/$USERNAME/.ssh/${VMPREFIX}.pub"
		# SSH追加設定
		for count in $(seq 1 $MAXVM) ; do
			line=$(sed -n ${count}P ./ipaddresslist)
		...
		for count in $(seq 1 $MAXVM); do
			ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${VMPREFIX}-${count} -t -t "sudo sed -i -e 's/SendEnv LANG LC_/#SendEnv LANG LC_/' /etc/ssh/ssh_config"
			ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${VMPREFIX}-${count} -t -t "echo 'en_US.UTF-8 UTF-8' >> /etc/locale.gen"
		done
		# ホストファイル事前バックアップ（PBSノード追加設定向け）
		echo "backup original hostfile"
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} "sudo cp /etc/hosts /etc/hosts.original""
		# PBSノード：マウント設定
		...
		if [ -f ./vmlist ]; then
			rm ./vmlist
			echo "recreating a new vmlist"
		fi
		if [ -f ./hostsfile ]; then
			rm ./hostsfile
			echo "recreating a new hostfile"
		fi
		if [ -f ./nodelist ]; then
			rm ./nodelist
			echo "recreating a new nodelist"
		fi
		# vmlist 作成
		numvm=1
		while [ $((numvm)) -lt $((MAXVM)) ]; do
		...
		echo "show current vmlist: `cat ./vmlist`"
		# ホストファイル作成準備：プライベートIPアドレス取得
		for count in $(seq 1 $MAXVM) ; do
			nodelist=$(az vm show -g $MyResourceGroup --name ${VMPREFIX}-${count} -d --query privateIps -o tsv)
			echo $nodelist >> nodelist
		done
		# ログインノードプライベートIPアドレス取得
		loginprivateip=$(az vm show -g $MyResourceGroup --name ${VMPREFIX}-login -d --query privateIps -o tsv)
		echo "$loginprivateip" >> nodelist
		pbsprivateip=$(az vm show -g $MyResourceGroup --name ${VMPREFIX}-pbs -d --query privateIps -o tsv)
		echo "$pbsprivateip" >> nodelist
		echo "show current nodelist: $(cat ./nodelist)"
		# ホストファイル作成
		paste ./nodelist ./vmlist > ./tmphostsfile
		# ダブルクォーテーション削除
		sed -i -e "s/\"//g" ./tmphostsfile
		# ファイルの重複行削除。列は2列まで想定
		cat  ./tmphostsfile | awk '!colname[$1]++{print $1,$2}' >> ./hostsfile
		cat ./hostsfile
		# テンポラリファイル削除
		rm ./tmphostsfile
### ===========================================================================
			...
			if [ ! -z "$checkssh" ]; then
				...
				if [ ! -z "$checkssh2" ]; then
					...
					for count in `seq 1 $MAXVM`; do
						...
						az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts "scp -o StrictHostKeyChecking=no -i /home/$USERNAME/${VMPREFIX} $USERNAME@${loginprivateip}:/home/$USERNAME/hostsfile /home/$USERNAME/"
						echo "コンピュートノード： az: ホストファイル更新"
						az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts "sudo cp /etc/hosts.original /etc/hosts"
						az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts "sudo cp /home/$USERNAME/hostsfile /etc/hosts"
						az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts "sudo cat /etc/hosts"
					done
				fi
			fi
		done
...
### ===========================================================================
		...
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo yum install --quiet -y hwloc-libs libICE libSM""
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo yum install --quiet -y libnl3""
		echo "installing libnl3"
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo yum install --quiet -y /home/$USERNAME/openpbs-execution-20.0.1-0.x86_64.rpm""
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo rpm -aq | grep openpbs""
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo /opt/pbs/libexec/pbs_habitat""
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo /opt/pbs/libexec/pbs_postinstall""
		# pbs.confファイル生成
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo sed -i -e 's/PBS_START_MOM=0/PBS_START_MOM=1/g' /etc/pbs.conf""
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo sed -i -e s/CHANGE_THIS_TO_PBS_SERVER_HOSTNAME/${VMPREFIX}-pbs/g /etc/pbs.conf""
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo cat /etc/pbs.conf""
		# openPBSクライアント：パーミッション設定
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo chmod 4755 /opt/pbs/sbin/pbs_iff""
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo chmod 4755 /opt/pbs/sbin/pbs_rcp""
		# openPBSクライアント：/var/spool/pbs/mom_priv/config コンフィグ設定
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo sed -i -e s/CHANGE_THIS_TO_PBS_SERVER_HOSTNAME/${VMPREFIX}-pbs/g /var/spool/pbs/mom_priv/config""
		for count in `seq 1 $MAXVM` ; do
			line=$(sed -n ${count}P ./ipaddresslist)
...
### ===========================================================================
		...
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo /etc/init.d/pbs start""
		vm1ip=$(cat ./ipaddresslist | head -n 1)
		ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${vm1ip} -t -t "grep pbs.sh /home/azureuser/.bashrc" > ./pbssh
		pbssh=$(cat ./pbssh)
		if [ -z "$pbssh" ]; then
			parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t 'echo 'source /etc/profile.d/pbs.sh' >> $HOME/.bashrc'"
		fi
		rm ./pbssh
...
### ===========================================================================
	...
	start )
		...
		prevm1ip=$(cat ./ipaddresslist | head -n 1)
		...
		if [ ! -z "$pbsvmname" ]; then
			...
			if [ -z "$pbsvmip" ]; then
				...
				az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-1 --command-id RunShellScript --scripts "sudo mount $DEBUG -t nfs ${pbsmountip}:/mnt/share /mnt/share"
				for count in $(seq 2 $MAXVM) ; do
				# コンピュートノード#1をマウント
					az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts "sudo mkdir -p /mnt/share"
					az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts "sudo chown $USERNAME:$USERNAME /mnt/share"
		...
		if [ -z "$pbsvmip" ]; then
			...
			if [ ! -z "$checkssh" ]; then
				...
				echo "nfs client mount status"
					for count in `seq 2 $MAXVM`; do
						line=$(sed -n ${count}P ./ipaddresslist)
						ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} -t -t $USERNAME@${line} "echo '########## host: ${VMPREFIX}-: 2 - ${count} ##########'"
	...
	remount )
		...
		if [ -f ./ipaddresslist ]; then
			...
			if [ -z "$pbsvmname" ]; then
				...
				parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} -t -t $USERNAME@{} "sudo mount -t nfs ${pbsmountip}:/mnt/share""
			fi
		else
			for count in $(seq 2 $MAXVM) ; do
...
echo "number of pingpong: $max"
echo "checkosver: $checkosver"
# run pingpong
case $checkosver in
	7.?.???? )
...
#				ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "md5sum /home/$USERNAME/openpbs-server-20.0.1-0.x86_64.rpm | cut -d ' ' -f 1" > md5server-remote
#				echo "md5server-remote: $md5server-remote"
#				while [ ! "$md5server"="6e7a7683699e735295dba6e87c6b9fd0" ]; do
#					ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "rm /home/$USERNAME/openpbs-server-20.0.1-0.x86_64.rpm"
...
#				md5client=$(ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "md5sum /home/$USERNAME/openpbs-client-20.0.1-0.x86_64.rpm | cut -d ' ' -f 1")
#				echo "md5client: $md5client"
#				while [ ! "$md5client"="7bcaf948e14c9a175da0bd78bdbde9eb" ]; do
#					ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "rm /home/$USERNAME//openpbs-client-20.0.1-0.x86_64.rpm"
...
#					ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "md5sum /home/$USERNAME/openpbs-server-20.0.1-0.x86_64.rpm | cut -d ' ' -f 1 > md5client-remote"
...
#				done
		...
		ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "ls -la $HOME"
...
### ===========================================================================
		...
		parallel $DEBUG -a ipaddresslist "scp -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} ./hostsfile $USERNAME@{}:/home/$USERNAME/"
		parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "cat /home/$USERNAME/hostsfile | sudo tee -a /etc/hosts""
		for count in $(seq 1 $MAXVM); do
			line=$(sed -n ${count}P ./ipaddresslist)
			ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${line} -t -t "cat /home/$USERNAME/hostsfile | sudo tee -a /etc/hosts"
			ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${line} -t -t "sudo cat /etc/hosts"
		done
### ===========================================================================
		# PBSプロセス起動
		# PBSノード起動＆$USERNAME環境変数設定
...
EOL
		...
		# PBSノードでの実施なのでipaddresslist から nodelistに変更
...
#!/bin/bash
PBSNODE=$(cat pbsvmip)
SSHKEY=$(grep VMPREFIX ./hpcbmtenvcreate-v2p3-test.sh | head -n 1 | cut -d "=" -f 2)
echo "SSHKEY: $SSHKEY"

ssh -i ~/.ssh/$SSHKEY azureuser@$PBSNODE
EOL
		# ログインノードへアクセス
		cat <<'EOL' >> sshloginvm.sh
#!/bin/bash
LOGINNODE=$(cat pbsvmip)
SSHKEY=$(grep VMPREFIX ./hpcbmtenvcreate-v2p3-test.sh | head -n 1 | cut -d "=" -f 2)
echo "SSHKEY: $SSHKEY"

ssh -i ~/.ssh/$SSHKEY azureuser@$LOGINNODE	
EOL
	;;
### ===========================================================================
	updatensg )
	...
	;;
		...
		# SSH VMアクセススクリプト
		cat <<'EOL' >> sshvm.sh
echo "need vm number!: ex. ./sshvm.sh 2"
#VMPREFIX=sample
#SSHKEYDIR=sample
#USERNAME=sample
...
echo "SSHKEYDIR: $SSHKEYDIR"
line=$(echo "$1")
vmip=$(sed -n ${line}P ./ipaddresslist)
echo "access pbulic ip: $vmip"
echo "accces username: $USERNAME"
ssh -i $SSHKEYDIR $USERNAME@${vmip}
EOL
		VMPREFIX=$(grep "VMPREFIX=" ${CMDNAME} | head -n 1 | cut -d "=" -f 2)
		sed -i -e "s/^#VMPREFIX=sample/VMPREFIX=$VMPREFIX/" ./sshvm.sh
		SSHKEYDIR=$(grep "SSHKEYDIR=" ${CMDNAME} | sed -n 2p | cut -d "=" -f 2)
		sed -i -e "s:^#SSHKEYDIR=sample:SSHKEYDIR=$SSHKEYDIR:" ./sshvm.sh
		USERNAME=$(grep "USERNAME=" ${CMDNAME} | head -n 1 | cut -d "=" -f 2)
		sed -i -e "s/^#USERNAME=sample/USERNAME=$USERNAME/" ./sshvm.sh
### ===========================================================================
		rm checkmount.sh
		# VMマウントチェックスクリプト
