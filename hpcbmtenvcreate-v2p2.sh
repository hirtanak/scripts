...
case $1 in
	create )
		...
		if [ $((numip)) -ne $((MAXVM)) ]; then
			echo "error!: you can not get all ip address list. please retry later."
			exit 1
		fi
		# 永続ディスクが必要な場合に設定可能
		...
		if [ -z "$checkssh" ]; then
			az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-1 --command-id RunShellScript \
				--scripts "sudo yum install --quiet -y nfs-utils epel-release htop && echo '/mnt/resource *(rw,no_root_squash,async)' >> /etc/exports"
			sleep 5
			...
			ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${vm1ip} -t -t "sudo yum install --quiet -y nfs-utils epel-release htop"
		...
		parallel $DEBUG -a ipaddresslist-tmp "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo yum install --quiet -y nfs-utils epel-release htop""
		...
		for count in `seq 2 $MAXVM` ; do
			...
			if [ ! -z "$checkssh" ]; then
				...
				ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${line} -t -t "sudo yum install --quiet -y nfs-utils epel-release htop"
				...
				az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts 'sudo yum install --quiet -y nfs-utils epel-release htop'
...
### ===========================================================================
		...
		if [ ! -z "$checkssh" ]; then
			...
			# SSHできないのでaz vm run-commandでのホストファイル転送・更新
			...
			az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-pbs --command-id RunShellScript --scripts \ 
				"scp -o StrictHostKeyChecking=no -i /home/$USERNAME/${VMPREFIX} $USERNAME@${loginprivateip}:/home/$USERNAME/hostsfile /home/$USERNAME/"
			...
			if [ ! -z "$checkssh" ]; then
				...
				if [ ! -z "$checkssh2" ]; then
					...
					for count in `seq 1 $MAXVM`; do
						...
						az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts \ 
							"scp -o StrictHostKeyChecking=no -i /home/$USERNAME/${VMPREFIX} $USERNAME@${loginprivateip}:/home/$USERNAME/hostsfile /home/$USERNAME/"
...
### ===========================================================================
		...
		pbssh=$(cat ./pbssh)
		if [ -z "$pbssh" ]; then
			parallel $DEBUG -a ipaddresslist "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 30' -i ${SSHKEYDIR} $USERNAME@{} -t -t 'echo 'source /etc/profile.d/pbs.sh' >> ~/.bashrc'"
		fi
		rm ./pbssh
...
#		vm1ip=$(head -1 ./ipaddresslist)
		impidir=$(ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${vm1ip} "ls /opt/intel/impi/")
		cat ./pingponglist
...
EOL
		...
		ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "sudo yum install --quiet -y nfs-utils epel-release htop"
...
#				done
		...
		ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "ls -la ~/"
