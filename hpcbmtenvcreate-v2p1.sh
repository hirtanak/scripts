...
LIMITEDIP=$(curl -s https://ifconfig.io)/32 #利用しているクライアントのグローバルIPアドレスを取得
CHECKIP=$(echo $LIMITEDIP | grep 503)
if [[ ! -z $CHECKIP ]]; then
	LIMITEDIP=$(curl -s https://ipinfo.io/ip)/32 #代替サイトでグローバルIPアドレスを取得
fi
	...
	create )
		...
		if [ -z "$checkssh" ]; then
			az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-1 --command-id RunShellScript \
				--scripts "sudo yum install -y nfs-utils epel-release htop && echo '/mnt/resource *(rw,no_root_squash,async)' >> /etc/exports"
			sleep 5
...
#			parallel $DEBUG -a ipaddresslist-tmp "ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${} -t "echo "$USERNAME ALL=NOPASSWD: ALL" | sudo tee -a /etc/sudoers""
		...
		parallel $DEBUG -a ipaddresslist-tmp "ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 180' -i ${SSHKEYDIR} $USERNAME@{} -t -t "sudo yum install --quiet -y nfs-utils epel-release htop""
		...
		for count in `seq 2 $MAXVM` ; do
			...
			if [ ! -z "$checkssh" ]; then
				...
				ssh -o StrictHostKeyChecking=no -i ${SSHKEYDIR} $USERNAME@${line} -t -t "sudo yum install -y nfs-utils epel-release htop"
				...
				az vm run-command invoke -g $MyResourceGroup --name ${VMPREFIX}-${count} --command-id RunShellScript --scripts 'sudo yum install -y nfs-utils epel-release htop'
...
EOL
		...
		cat ./hosttmpfile2 > ./hosttmpfile3
		# ファイルの重複行削除。列は2列まで想定
		cat  ./hosttmpfile3 | awk '!colname[$1]++{print $1,$2}' >> ./hostsfile
		cat ./hostsfile
		# テンポラリファイル削除
		rm ./hosttmpfile
		rm ./hosttmpfile2
...
### ===========================================================================
		...
		ssh -o StrictHostKeyChecking=no -o 'ConnectTimeout 180' -i ${SSHKEYDIR} $USERNAME@${pbsvmip} -t -t "sudo su - && sudo bash /home/$USERNAME/setuppbs.sh"
