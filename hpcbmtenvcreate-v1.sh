...
EOL
	...
	start )
		...
		echo "starting VM ${VMPREFIX} compute nodes"
		...
		echo "mount nfs server @ ${VMPREFIX}-1"
		cat ./ipaddresslist
		tmpfile=$(stat ./${VMPREFIX} -c '%a')
		case $tmpfile in
		600 )
			seq 2 $MAXVM | parallel -v -a ipaddresslist "ssh -o StrictHostKeyChecking=no -i ./${VMPREFIX} -t $USERNAME@{} "sudo mount -t nfs ${mountip}:/mnt/resource /mnt/resource""
			;;
		7** )
			# 何らかの事情でファイルパーミッションが設定できていない場合、~/.ssh/ディレクトリを利用する
			cp ./${VMPREFIX} ~/.ssh/
			chmod 600 ~/.ssh/${VMPREFIX}
			seq 2 $MAXVM | parallel -v -a ipaddresslist "ssh -o StrictHostKeyChecking=no -i ~/.ssh/${VMPREFIX} -t $USERNAME@{} "sudo mount -t nfs ${mountip}:/mnt/resource /mnt/resource""
		;;
		esac
####for count in $(seq 2 $MAXVM) ; do
		...
		if [ -f ./ipaddresslist ]; then
			...
			case $tmpfile in
				600 )
					...
					seq 1 $MAXVM | parallel -v -a ipaddresslist "ssh -o StrictHostKeyChecking=no -i ~/.ssh/${VMPREFIX} $USERNAME@{} 'sudo mount -t nfs ${mountip}:/mnt/resource /mnt/resource'"
				;;
				7** )
					ssh -o StrictHostKeyChecking=no -i ~/.ssh/${VMPREFIX} $USERNAME@${ipaddresstmp} 'showmount -e'
					seq 1 $MAXVM | parallel -v -a ipaddresslist "ssh -o StrictHostKeyChecking=no -i ~/.ssh/${VMPREFIX} $USERNAME@{} 'sudo mount -t nfs ${mountip}:/mnt/resource /mnt/re    source'"
				;;
			esac
		else
			for count in $(seq 2 $MAXVM) ; do
